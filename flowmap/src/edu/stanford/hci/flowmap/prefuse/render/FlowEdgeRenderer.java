...
public abstract class FlowEdgeRenderer extends DefaultEdgeRenderer {
	...
	public FlowEdgeRenderer(UserOptions userOptions, QueryRecord flowRecord) {
		super();
	...
	public FlowScale getUserScaling() {
		FlowScale scale;
		if (userOptions.getBoolean(UserOptions.LINEAR_SCALE))
			scale = linearScale;
		else if (userOptions.getBoolean(UserOptions.LOG_SCALE))
			scale = logScale;
		else
			scale = polyScale;
		return scale;
